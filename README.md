...
在**项目目录**创建 `config.yaml`，写入你要用的视觉模型：
...
| Anthropic | `anthropic` | 不需要 | `claude-3-5-sonnet-20241022` | [console.anthropic.com](https://console.anthropic.com/) |
| vLLM / LM Studio / LocalAI | `openai` | 你部署的地址（如 `http://localhost:8000/v1`） | 你的模型名 | 自行部署，api_key 留空 |

**Anthropic 用户**，`type` 改为 `anthropic`，删掉 `base_url` 行：
...
**1. 确保已创建 config.yaml（第二步），然后启动服务：**
...
编辑项目根目录的 `.mcp.json`（或全局 `~/.claude.json`）：
...
>
> 其他服务商只需改 `BASE_URL`、`API_KEY`、`MODEL` 三个值。**Anthropic 用户**额外把 `PROVIDER_TYPE` 改为 `anthropic`，并删掉 `PROVIDER_BASE_URL` 行。
...
图片路径不在白名单内。在 config.yaml 的 `allowed_paths` 中添加对应目录。
